...
interface HeaderProps {
  ...
  // Opens the mobile sidebar drawer. Only rendered as a button below the
  // lg breakpoint — desktop layout/behavior is unaffected.
...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="sticky top-4 my-4 mx-4 lg:mx-0 lg:mr-4 z-10 flex items-center justify-between gap-2 sm:gap-4 px-3 sm:px-6 py-3.5 bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
      {/* Mobile-only hamburger button to open the sidebar drawer */}
      {onOpenMobileMenu && (
        <button
          onClick={onOpenMobileMenu}
          className="lg:hidden shrink-0 p-2 rounded-2xl text-slate-600 hover:text-slate-900 bg-slate-50 hover:bg-slate-100 border border-slate-200/70 transition-colors cursor-pointer"
      ...
      <div className="flex items-center gap-4 min-w-0">
        <div className="min-w-0">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span
              className={`hidden sm:flex items-center gap-1 px-2.5 py-0.5 text-[10px] font-extrabold uppercase border rounded-full shrink-0 ${
                isSuperAdmin
          ...
          <p className="hidden sm:flex text-xs text-slate-600 items-center gap-1.5 mt-0.5">
      ...
      <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
        ...
        <div ref={notificationsRef} className="relative">
          ...
          {/* Notifications Panel */}
          {showNotifications && (
            <div className="absolute right-0 top-full mt-2 w-[calc(100vw-2rem)] max-w-80 bg-white/95 backdrop-blur-xl border border-slate-200 rounded-3xl shadow-2xl p-4 z-50">
