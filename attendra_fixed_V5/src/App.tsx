...
export default function App() {
  ...
  const loadDatabaseData = async () => {
    try {
      const [
        ...
        api.getLeavePolicies().catch(() => null),
        api.getAnnouncements().catch(() => null),
        api.getHolidays().catch(() => null),
        api.getAuditLogs().catch(() => null),
        api.getNotifications().catch(() => null),
        api.getUserAccounts().catch(() => null),
        api.getRegistrationRequests().catch(() => null),
        api.getLatePenaltyRule().catch(() => null),
  ...
  const empAvgHours = scopedAttendance.length > 0
    ? scopedAttendance.reduce((acc, curr) => acc + (curr.workHours || 8.0), 0) / scopedAttendance.length
    : 8.0;
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-800 font-sans relative overflow-x-hidden selection:bg-blue-500 selection:text-white">
      ...
      <div className="flex min-h-screen max-w-[1700px] mx-auto">
        ...
        <main className="flex-1 flex flex-col min-w-0 pr-4 pl-1">
          {/* Top Sticky Header */}
          <Header
            currentUser={currentUser}
          ...
          <div className="flex-1 pb-10 px-2 space-y-6">
            {/* 1. DASHBOARD VIEW */}
