...
import { Employee, AttendanceRecord, LeaveRequest, LeavePolicy, UserRole } from '../../types';
...
export const EmployeeProfileModal: React.FC<EmployeeProfileModalProps> = ({
  ...
  const totalHours = empAttendance.reduce((acc, r) => acc + (r.workHours || 0), 0);
  ...
  const leaveTypesSummary = [
    { type: 'Casual Leave', used: empLeaves.filter(l => l.leaveType === 'casual').reduce((sum, l) => sum + l.totalDays, 0), total: employee.leaveBalance.casual, color: 'bg-purple-50 text-purple-700 border-purple-200' },
    { type: 'Sick Leave', used: empLeaves.filter(l => l.leaveType === 'sick').reduce((sum, l) => sum + l.totalDays, 0), total: employee.leaveBalance.sick, color: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
    { type: 'Annual Leave', used: empLeaves.filter(l => l.leaveType === 'annual').reduce((sum, l) => sum + l.totalDays, 0), total: employee.leaveBalance.annual, color: 'bg-blue-50 text-blue-700 border-blue-200' },
    { type: 'Unpaid Leave', used: empLeaves.filter(l => l.leaveType === 'unpaid').reduce((sum, l) => sum + l.totalDays, 0), total: employee.leaveBalance.unpaid || 10, color: 'bg-slate-50 text-slate-700 border-slate-200' },
    { type: 'Emergency Leave', used: empLeaves.filter(l => l.leaveType === 'emergency').reduce((sum, l) => sum + l.totalDays, 0), total: employee.leaveBalance.emergency, color: 'bg-amber-50 text-amber-700 border-amber-200' },
    { type: 'Maternity Leave', used: empLeaves.filter(l => l.leaveType === 'maternity').reduce((sum, l) => sum + l.totalDays, 0), total: employee.leaveBalance.maternity || 90, color: 'bg-pink-50 text-pink-700 border-pink-200' },
    { type: 'Paternity Leave', used: empLeaves.filter(l => l.leaveType === 'paternity').reduce((sum, l) => sum + l.totalDays, 0), total: employee.leaveBalance.paternity || 12, color: 'bg-indigo-50 text-indigo-700 border-indigo-200' },
    { type: 'Half Day Leave', used: empLeaves.filter(l => l.leaveType === 'half_day').reduce((sum, l) => sum + l.totalDays, 0), total: employee.leaveBalance.half_day, color: 'bg-teal-50 text-teal-700 border-teal-200' },
  ];
        ...
        <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-indigo-950 text-white p-6 relative shrink-0">
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start gap-5">
            ...
            <div className="flex-1 text-center md:text-left space-y-1">
              ...
              <p className="text-xs font-semibold text-blue-300">{employee.role}</p>
          ...
          {activeTab === 'monthly' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3 text-center">
                ...
                <div className="p-3 bg-white rounded-2xl border border-slate-200 shadow-xs">
                  <span className="block text-xl font-black text-rose-500">1</span>
                  <span className="text-[10px] font-bold text-slate-500">Absent</span>
                </div>
                <div className="p-3 bg-white rounded-2xl border border-slate-200 shadow-xs">
                  <span className="block text-xl font-black text-purple-600">3</span>
                  <span className="text-[10px] font-bold text-slate-500">Leave</span>
                </div>
                <div className="p-3 bg-white rounded-2xl border border-slate-200 shadow-xs">
                  <span className="block text-xl font-black text-teal-600">1</span>
                  <span className="text-[10px] font-bold text-slate-500">Half Day</span>
                </div>
                <div className="p-3 bg-white rounded-2xl border border-slate-200 shadow-xs">
                  <span className="block text-xl font-black text-blue-600">4</span>
                  <span className="text-[10px] font-bold text-slate-500">Remote WFH</span>
                </div>
                <div className="p-3 bg-white rounded-2xl border border-slate-200 shadow-xs">
                  <span className="block text-xl font-black text-indigo-600">1</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 text-center">
                <div className="p-3.5 bg-blue-50 border border-blue-100 rounded-2xl">
                  <span className="block text-lg font-black text-blue-900">182.5 hrs</span>
                ...
                <div className="p-3.5 bg-amber-50 border border-amber-100 rounded-2xl">
                  <span className="block text-lg font-black text-amber-900">35 mins</span>
                ...
                <div className="p-3.5 bg-rose-50 border border-rose-100 rounded-2xl">
                  <span className="block text-lg font-black text-rose-900">15 mins</span>
                ...
                <div className="p-3.5 bg-emerald-50 border border-emerald-100 rounded-2xl">
                  <span className="block text-lg font-black text-emerald-900">6.5 hrs</span>
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-[9px] block mt-0.5">
                          {isWeekend ? 'Off' : isLeave ? 'Leave' : isLate ? 'Late' : 'Present'}
                        </span>
                      </div>
                    );
                  })}
              ...
              <div className="bg-gradient-to-r from-purple-900 via-indigo-900 to-slate-900 text-white p-5 rounded-3xl shadow-md flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div>
                  ...
                  <h3 className="text-2xl font-black text-white mt-1">
                    {empLeaves.reduce((sum, l) => sum + l.totalDays, 0)} Days Total Leave Taken
                  </h3>
                  <p className="text-xs text-purple-200/80 mt-0.5">
                    Casual: {empLeaves.filter(l=>l.leaveType==='casual').reduce((sum, l)=>sum+l.totalDays,0)}d • Sick: {empLeaves.filter(l=>l.leaveType==='sick').reduce((sum, l)=>sum+l.totalDays,0)}d • Annual: {empLeaves.filter(l=>l.leaveType==='annual').reduce((sum, l)=>sum+l.totalDays,0)}d • Unpaid: {empLeaves.filter(l=>l.leaveType==='unpaid').reduce((sum, l)=>sum+l.totalDays,0)}d • Emergency: {empLeaves.filter(l=>l.leaveType==='emergency').reduce((sum, l)=>sum+l.totalDays,0)}d
                  </p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <div className="px-3 py-1.5 bg-white/10 backdrop-blur-md rounded-2xl border border-white/15 text-center">
                    ...
                    <span className="text-sm font-black text-white">{empLeaves.filter(l=>l.leaveType==='casual').reduce((sum, l)=>sum+l.totalDays,0)} Days</span>
                  </div>
                  <div className="px-3 py-1.5 bg-white/10 backdrop-blur-md rounded-2xl border border-white/15 text-center">
                    <span className="block text-xs font-bold text-emerald-200">Sick</span>
                    <span className="text-sm font-black text-white">{empLeaves.filter(l=>l.leaveType==='sick').reduce((sum, l)=>sum+l.totalDays,0)} Days</span>
                  </div>
                  <div className="px-3 py-1.5 bg-white/10 backdrop-blur-md rounded-2xl border border-white/15 text-center">
                    <span className="block text-xs font-bold text-blue-200">Annual</span>
                    <span className="text-sm font-black text-white">{empLeaves.filter(l=>l.leaveType==='annual').reduce((sum, l)=>sum+l.totalDays,0)} Days</span>
                  </div>
                  <div className="px-3 py-1.5 bg-white/10 backdrop-blur-md rounded-2xl border border-white/15 text-center">
                    <span className="block text-xs font-bold text-slate-300">Unpaid</span>
                    <span className="text-sm font-black text-white">{empLeaves.filter(l=>l.leaveType==='unpaid').reduce((sum, l)=>sum+l.totalDays,0)} Days</span>
                  </div>
                  <div className="px-3 py-1.5 bg-white/10 backdrop-blur-md rounded-2xl border border-white/15 text-center">
                    <span className="block text-xs font-bold text-amber-200">Emergency</span>
                    <span className="text-sm font-black text-white">{empLeaves.filter(l=>l.leaveType==='emergency').reduce((sum, l)=>sum+l.totalDays,0)} Days</span>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 space-y-5">
            ...
            <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-2xl border border-slate-100">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-slate-500 truncate">{employee.role} • {employee.department}</p>
